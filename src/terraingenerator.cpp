...
TerrainGenerator::TerrainGenerator(QGraphicsScene *scene, QObject *parent)
    : QObject(parent),
      m_scene(scene)
{
    // 初始化随机种子    m_seed = QDateTime::currentMSecsSinceEpoch();
    m_randomGenerator = QRandomGenerator(m_seed);
...
void TerrainGenerator::generateChunk(int chunkIndex)
{
    ...
    for (int r = 0; r < rockCount; ++r)
    {
        ...
        qreal globalX = chunkIndex * CHUNK_WIDTH + x; // 检查斜率 - 允许初始区域也生成石头
        qreal slope = getTerrainSlope(globalX);
        if (qAbs(slope) > MAX_SLOPE_FOR_ROCK)
            continue; // 移除chunkIndex==0的限制
    ...
    { // 跳过第一个地形块
        ...
        if (qAbs(npcSlope) <= MAX_SLOPE_FOR_ROCK)
        { // 使用与石头相同的斜率限制
            // 决定生成企鹅还是雪怪 (4:1比例)
...
void TerrainGenerator::generateChunkThreadSafe(int chunkIndex)
{
    ...
    } // 生成NPC的位置数据（不创建实体）- 支持企鹅和雪人4:1比例，包含初始区域
    QVector<NPCGenerationData> npcDataList;
    if (chunkIndex >= 0)
    { // 包含初始区域（chunkIndex=0），让游戏开始就有生机
        // 随机选择生成位置（块内）
        qreal npcX = QRandomGenerator::global()->bounded(CHUNK_WIDTH / 4, CHUNK_WIDTH * 3 / 4);
    ...
    } // 生成树木（在地面上）- 增加密度两倍，让雪原更加浪漫
    QVector<TreeGenerationData> treeDataList;
    int treeCount = QRandomGenerator::global()->bounded(4, 9); // 每个区块4-8棵树（密度翻倍）
    QVector<qreal> treeXs;                                     // 记录已生成树木的x坐标，避免重叠

    for (int t = 0; t < treeCount; ++t)
    {
        // 随机选择树木位置（避开NPC和石头区域）
        qreal treeX = QRandomGenerator::global()->bounded(0, CHUNK_WIDTH);

        // 检查与已生成物体的距离（包括石头和其他树木）
        bool tooClose = false; // 检查与石头的距离
        for (const RockGenerationData &rock : rockDataList)
        {
            if (qAbs(treeX - rock.localX) < 50)
            { // 树木与石头至少50像素距离（进一步减少间距）
                tooClose = true;
                break;
            }
        }
        // 检查与其他树木的距离
        for (qreal prevTreeX : treeXs)
        {
            if (qAbs(treeX - prevTreeX) < 30)
            { // 树木之间至少30像素距离（减少间距让树林更密集）
                tooClose = true;
                break;
            }
        }
        ...
        // 使用与石头相同的方法计算高度和斜率
        int i = 0;
        while (i < points.size() && points[i].x() < treeX)
        {
            i++;
        }
        ...
        if (i > 0 && i < points.size())
        {
            ...
            if (qAbs(slope) <= MAX_SLOPE_FOR_ROCK)
            {
                qreal height = y1 + slope * (treeX - x1);
                qreal globalTreeX = chunkIndex * CHUNK_WIDTH + treeX; // 随机树木大小（增大高度和宽度）
                qreal treeWidth = QRandomGenerator::global()->bounded(40, 70);
                qreal treeHeight = QRandomGenerator::global()->bounded(100, 160);
                qreal treeY = height; // 树木底部对齐地面（TreeEntity::setPosition会自动调整位置）

                // 保存树木数据
                TreeGenerationData treeData;
                treeData.localX = treeX;
                treeData.globalX = globalTreeX;
                treeData.y = treeY;
        ...
        } // 生成云朵（在天空中）- 增加密度两倍，营造更丰富的天空
        QVector<CloudGenerationData> cloudDataList;
        int cloudCount = QRandomGenerator::global()->bounded(2, 5); // 每个区块2-4朵云（密度翻倍）

        for (int c = 0; c < cloudCount; ++c)
        {
            // 随机云朵位置
            qreal cloudX = QRandomGenerator::global()->bounded(0, CHUNK_WIDTH);
            qreal globalCloudX = chunkIndex * CHUNK_WIDTH + cloudX;

            // 计算该位置的地形高度
            int i = 0;
            while (i < points.size() && points[i].x() < cloudX)
            {
                i++;
            }

            qreal localGroundHeight = 300; // 默认高度
            if (i > 0 && i < points.size())
            {
                qreal x1 = points[i - 1].x();
                qreal y1 = points[i - 1].y();
                ...
                qreal dx = x2 - x1;
                qreal slope = (qAbs(dx) > 1e-9) ? ((y2 - y1) / dx) : 0.0;
                localGroundHeight = y1 + slope * (cloudX - x1);
            }

            // 云朵在地面上方100-150像素（只比树稍高）
            qreal cloudY = localGroundHeight - QRandomGenerator::global()->bounded(120, 180);

            // 随机云朵大小，稍微小一点
            qreal cloudWidth = QRandomGenerator::global()->bounded(60, 120);
            qreal cloudHeight = QRandomGenerator::global()->bounded(30, 60);

            // 保存云朵数据
            CloudGenerationData cloudData;
            cloudData.localX = cloudX;
            cloudData.globalX = globalCloudX;
            cloudData.y = cloudY;
            cloudData.width = cloudWidth;
            cloudData.height = cloudHeight;
            cloudDataList.append(cloudData);
        }
        // 存储生成的路径、点数据、石头数据、NPC数据、树木数据和云朵数据
        {
            QMutexLocker locker(&m_mutex);
            m_generatedPaths[chunkIndex] = path;
            m_chunkPoints[chunkIndex] = points;
            ...
            m_generatedTrees[chunkIndex] = treeDataList;
            m_generatedClouds[chunkIndex] = cloudDataList;
        }
    }
}
// 在主线程中完成将区块添加到场景
void TerrainGenerator::addChunkToScene(int chunkIndex)
{
    ...
    // 根据预先计算的数据创建树木实体
    if (m_generatedTrees.contains(chunkIndex))
    {
        const QVector<TreeGenerationData> &treeDataList = m_generatedTrees[chunkIndex];
        for (const TreeGenerationData &treeData : treeDataList)
        {
            TreeEntity *tree = new TreeEntity(treeData.width, treeData.height);
            tree->setPosition(QPointF(treeData.globalX, treeData.y));

            // 添加到场景和存储列表
            m_scene->addItem(tree);
            m_trees.append(tree);

            DEBUG_LOG(QString("创建树木 at chunk %1, position (%2, %3), size (%4x%5)")
                          .arg(chunkIndex)
                          .arg(treeData.globalX)
                          .arg(treeData.y)
                          .arg(treeData.width)
                          .arg(treeData.height));
        }
        ...
        // 处理完后移除树木数据
        m_generatedTrees.remove(chunkIndex);
    }

    // 根据预先计算的数据创建云朵实体
    if (m_generatedClouds.contains(chunkIndex))
    {
        const QVector<CloudGenerationData> &cloudDataList = m_generatedClouds[chunkIndex];
        for (const CloudGenerationData &cloudData : cloudDataList)
        {
            CloudEntity *cloud = new CloudEntity(cloudData.width, cloudData.height);
            cloud->setPosition(QPointF(cloudData.globalX, cloudData.y));

            // 启动云朵的漂浮动画
            cloud->startFloating();

            // 添加到场景和存储列表
            m_scene->addItem(cloud);
            m_clouds.append(cloud);

            DEBUG_LOG(QString("创建云朵 at chunk %1, position (%2, %3), size (%4x%5)")
                          .arg(chunkIndex)
                          .arg(cloudData.globalX)
                          .arg(cloudData.y)
                          .arg(cloudData.width)
                          .arg(cloudData.height));
        }
...
void TerrainGenerator::removeDistantChunks(int currentChunk)
{
    ...
    { // 移除石头
        ...
        {
            TreeEntity *tree = m_trees[i];
            if (tree->x() >= index * CHUNK_WIDTH && tree->x() < (index + 1) * CHUNK_WIDTH)
            {
                m_scene->removeItem(tree);
                delete tree;
                m_trees.remove(i);
            }
        }
        ...
        {
            CloudEntity *cloud = m_clouds[i];
            if (cloud->x() >= index * CHUNK_WIDTH && cloud->x() < (index + 1) * CHUNK_WIDTH)
            {
                cloud->stopFloating(); // 停止动画
                m_scene->removeItem(cloud);
                delete cloud;
                m_clouds.remove(i);
            }
        } // 移除地形块
...
void TerrainGenerator::clearAllResources()
{
    ...
    // 清理云朵
    for (CloudEntity *cloud : m_clouds)
    {
        if (cloud)
        {
            cloud->stopFloating(); // 停止动画
